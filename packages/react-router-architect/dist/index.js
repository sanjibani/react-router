...
* React Router.
*/
function createRequestHandler({ build, getLoadContext, mode = process.env.NODE_ENV, useRequestContextDomainName = false }) {
	let handleRequest = createRequestHandler$1(build, mode);
	return async (event) => {
		return sendReactRouterResponse(await handleRequest(createReactRouterRequest(event, useRequestContextDomainName), await getLoadContext?.(event)));
	};
}
function createReactRouterRequest(event, useRequestContextDomainName = false) {
	let [hostname, portStr] = (useRequestContextDomainName ? event.requestContext.domainName || event.headers.host || "" : event.headers["x-forwarded-host"] || event.headers.host || "").split(":");
